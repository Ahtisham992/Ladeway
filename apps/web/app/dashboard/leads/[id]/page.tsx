...
export default async function LeadDetailPage({ params }: { params: { id: string } }) {
  ...
  if (!res.ok) {
    ...
    return <div className="p-8 text-red-500">Failed to load lead data.</div>;
  }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-4">
      <Link href="/dashboard" className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors">
        <ArrowLeft size={16} />
        Back to Leads
      </Link>
