...
export function LeadPipeline({ token }: { token: string }) {
 ...
 <h1 className="text-2xl font-bold text-primary ">Lead Pipeline</h1>
 <button
 onClick={fetchLeads}
 disabled={loading}
 className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-secondary bg-white border border-border rounded-lg hover:bg-background transition-colors :bg-slate-700"
 >
 ...
 <span className="text-sm font-medium text-secondary">Tier:</span>
 <select
 value={tierFilter}
 ...
 <TableCell>
 <div className="font-medium text-primary ">{lead.contactName || 'Unknown'}</div>
 <div className="text-xs text-secondary">{lead.contactEmail || lead.contactPhone || 'No contact info'}</div>
 </TableCell>
 <TableCell>
 ...
 <p className="text-sm text-secondary truncate " title={lead.summary}>
 {lead.summary}
 </p>
 </TableCell>
 <TableCell>
 <Badge tier={lead.tier as any || 'DEFAULT'}>
 {lead.tier || 'NONE'}
 </Badge>
 </TableCell>
 <TableCell>
 ...
 <TableCell className="text-sm text-secondary whitespace-nowrap">
 ...
 className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-secondary bg-surface-alt rounded-md hover:bg-slate-200 hover:text-primary transition-colors :bg-slate-700 :text-white"
 >
 View
 <ArrowRight size={14} />
 </Link>
